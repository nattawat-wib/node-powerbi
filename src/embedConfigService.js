 ...
 */
async function getEmbedInfo(body) {
    // Get the Report Embed details
    try {

        // Get report details and embed token
        // const embedParams = await getEmbedParamsForSingleReport(config.workspaceId, config.reportId, config.type);
        const embedParams = await getEmbedParamsForSingleReport(body.workspaceId, body.sourceId, body.type);
        ...
        // console.log("GET EMBED INFO: ", err);
        return {
            'status': err.status,
...
async function getEmbedParamsForSingleReport(workspaceId, sourceId, sourceType, additionalDatasetId) {
    ...
    reportEmbedConfig.embedToken = await getEmbedTokenForSingleReportSingleWorkspace(sourceId, datasetIds, workspaceId, sourceType);
